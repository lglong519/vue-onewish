<template>
	<div>
		<!--音乐控制器  -->
		<view wx:if='{{currAudio[0].url}}' class='playcontroller {{hide&&"hide"}} {{show&&"show"}} {{type !=="articleZH" && type !=="articleEN" && "bg-transparent"}} {{rollup && "rollup"}}'>
			<view wx:if='{{!rollup}}' class='flex-row'>
		...
		<icon wx:if='{{showZoom}}' class='material-icons text-30 text-grey zoom {{type !=="articleZH" && type !=="articleEN" && "text-white"}}' bindtap='zoom'>{{hideTabBar?"select_all":"zoom_out_map"}}</icon>
								...
								<block wx:for='{{art.sections}}' wx:for-index='j' wx:key='time'>
									<view class="weui-article__section mb-2">
										...
										<view wx:if='{{item.time}}' id='{{currPart==item.time? "currentPart":""}}' class='weui-article__h3 {{currPart==item.time && onPlay?"text-blue-2":"text-grey-6"}}'>
											{{item.time}}
											<icon class='material-icons {{currPart==item.time && onPlay?"text-green-0":"text-grey-1"}}' bindtap='playSection' data-art-id='{{art.id}}' data-art-url='{{art.url}}' data-art-time='{{item.time}}'>{{currPart==item.time && onPlay && currentTimeFormat[4]%2==0 ?"volume_mute":"volume_down"}}</icon>
										</view>
										<!--文本  -->
		...
		<view wx:else class="page page-music flex-column align-center">
			<view class='z-index disc-wrapper' animation="{{animation}}" bindtap='playControl'>
			...
			<view class='z-index mt-4 text-white'>{{currAudio[0].author}}</view>
